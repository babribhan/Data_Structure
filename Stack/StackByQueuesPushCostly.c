...
{  printf("Push\n");
   enqueue(&(stack->suppQueue), value);
   while (!isEmpty(stack->actQueue))
   {
       enqueue(&(stack->suppQueue),dequeue(&(stack->actQueue)));
   }
   queue_t *temp = stack->actQueue;
   stack->actQueue = stack->suppQueue;
   stack->suppQueue = temp;
}
